...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to list profiles', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, description, path, model, providerId, gatewayUrl, skillCount, envStatus, isActive } = body;
    ...
    const profile = await db.profile.create({
      data: {
        ...
        envStatus: envStatus ? JSON.stringify(envStatus) : '{}',
